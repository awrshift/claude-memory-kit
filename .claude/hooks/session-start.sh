...
if [ -d "$PROJECT_DIR/projects" ]; then
    for journal in "$PROJECT_DIR"/projects/*/JOURNAL.md; do
        if [ -f "$journal" ]; then
            ...
            ACTIVE=$(grep -c "Status:.*IN PROGRESS\|Status:.*TODO\|Status:.*BLOCKED" "$journal" 2>/dev/null || echo 0)
            DONE=$(grep -c "^- \*\*T-" "$journal" 2>/dev/null || echo 0)

            J_MTIME=$(stat -f '%m' "$journal" 2>/dev/null || stat -c '%Y' "$journal" 2>/dev/null)
            J_STALE=""
            if [ -n "$J_MTIME" ]; then
                J_DAYS=$(( (NOW - J_MTIME) / 86400 ))
                [ "$J_DAYS" -ge 5 ] && J_STALE=" !! STALE ($J_DAYS days)"
            fi

            echo "- $PROJECT_NAME: $ACTIVE active, $DONE completed$J_STALE"
        fi
    done
else
